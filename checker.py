...
class SemanticChecker:
    ...
    @multimethod
    def visit(self, node: NewExpr):
        class_decl = self.classes.get(node.class_name)
        if not class_decl:
            self.report(f"Clase '{node.class_name}' no definida", node.lineno)
            node.type = "error"
            return "error"

        ctor = self._get_method(node.class_name, '__init__')
        if ctor is not None:
            if not self._check_callable_args(f"'{node.class_name}.__init__'", ctor.params, node.args, node.lineno):
                node.type = "error"
                return "error"
        elif node.args:
            self.report(f"La clase '{node.class_name}' no define constructor y recibió argumentos", node.lineno)
            node.type = "error"
            return "error"

        node.type = node.class_name
        return node.class_name

    @multimethod
    def visit(self, node: MemberAccess):
        obj_type = self.visit(node.obj)
        if obj_type == "error":
            node.type = "error"
            return "error"

        class_decl = self.classes.get(obj_type)
        if class_decl is None:
            self.report(f"Acceso a miembro sobre tipo no-clase '{obj_type}'", node.lineno)
            node.type = "error"
            return "error"

        field = self._get_field(obj_type, node.member)
        if field is not None:
            node.type = field.type_name
            return field.type_name

        method = self._get_method(obj_type, node.member)
        if method is not None:
            self.report(f"El método '{obj_type}.{node.member}' debe invocarse con paréntesis", node.lineno)
            node.type = "error"
            return "error"

        self.report(f"Miembro '{node.member}' no existe en clase '{obj_type}'", node.lineno)
        node.type = "error"
        return "error"
    ...
    def visit(self, node: MethodCall):
        ...
        class_decl = self.classes.get(obj_type)
        if class_decl is None:
            self.report(f"Llamada a método sobre tipo no-clase '{obj_type}'", node.lineno)
            node.type = "error"
            return "error"

        method = self._get_method(obj_type, node.method)
        if method is None:
            self.report(f"Método '{node.method}' no existe en clase '{obj_type}'", node.lineno)
            node.type = "error"
            return "error"

        if not self._check_callable_args(f"'{obj_type}.{node.method}'", method.params, node.args, node.lineno):
            node.type = "error"
            return "error"
